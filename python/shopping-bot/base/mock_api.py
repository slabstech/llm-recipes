from fastapi import FastAPI, HTTPException, Depends, Header, Request  # Add Request import
...
from models import LoginRequest, UserResponse, OrderRequest, OrderResponse, ProcessOrderRequest
...
async def process_order_endpoint(request: ProcessOrderRequest, http_request: Request, current_user: Optional[str] = Depends(get_current_user, use_cache=False)):
    ...
    if current_user:
        ...
        if authorization.startswith("Bearer "):
            token = authorization.split(" ")[1]
    response = await process_order(request.session_id, request.user_input, request.username, request.password, token, RESTAURANTS)
    return response
