...
def fetch_menu_from_api(token=None):
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched menu. Open restaurants: {len(open_restaurants)}"
        )
        return None, open_restaurants
    except requests.RequestException as e:
        logger.error(
            f"Failed to fetch menu from API: {str(e)} - Response: {e.response.text if e.response else 'No response'}"
        )
        return str(e), {}
