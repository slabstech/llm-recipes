...
def fetch_menu_from_api():
    try:
        ...
        return f"Failed to fetch menu from API: {e}", {}  # Return error message and empty dict
...
def confirm_order(current_order, restaurants):
    ...
    if is_single_restaurant(current_order):
        ...
        return f"{summary}\n\nYour order contains items from multiple restaurants. Typically, orders are from a single restaurant. Please confirm if you want to proceed.", False
...
def process_final_order(current_order, restaurants, confirm_multi_restaurant):
    ...
    if not is_single_restaurant(current_order) and not confirm_multi_restaurant:
        return f"{summary}\n\nOrder not confirmed due to multiple restaurants. Please confirm to proceed.", False
...
with gr.Blocks(title="Zomato Order Bot") as demo:
    ...
    with gr.Row():
        order_input = gr.Textbox(label="What would you like to order? (e.g., 'I want 2 butter chickens and a veg pizza')", placeholder="Enter your order here")
    ...
    multi_restaurant_confirmed = gr.Checkbox(label="Confirm order from multiple restaurants", visible=False)
    ...
    def load_restaurants():
        ...
        return "Restaurants loaded successfully.", restaurants, "Add items to your order."

    demo.load(load_restaurants, outputs=[order_feedback, restaurants_state, confirm_output])
