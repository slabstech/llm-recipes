...
def chatbot_response(user_input, history, order, restaurants, awaiting_confirmation):
    ...
    if awaiting_confirmation:
        if user_input in ["yes", "y"]:
            ...
            return (
                "Order cancelled. Please order from a single restaurant. What would you like to order next?",
                {},
                False,
            )
        else:
            return (
                "Please respond with 'yes' or 'no' to confirm the order.",
                order,
                True,
            )
...
def load_initial_data():
    ...
    if error:
        return error, {}, {}
    return (
        "Welcome to the Food Order Bot! What would you like to order? (e.g., 'I want 2 butter chickens and a veg pizza')",
        {},
        restaurants,
    )
...
with gr.Blocks(title="Food Order Bot") as demo:
    ...
    chat_input = gr.Textbox(
        placeholder="Type your order here or 'done' to finish",
    ...
    def chat_wrapper(user_input, history):
        ...
        response, new_order, awaiting = chatbot_response(
            user_input, history, order_state.value, restaurants_state.value, False
        )
    ...
    chat_input.submit(
        chat_wrapper, inputs=[chat_input, chatbot], outputs=[chatbot, chat_input]
    )
