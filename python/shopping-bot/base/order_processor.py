...
async def execute_tool_call(
    tool_call, session_data: Dict, restaurants_data: Dict
) -> str:
    ...
    async with httpx.AsyncClient() as client:
        if function_name == "login":
            ...
            response = await client.post(
                config.LOGIN_API_URL, json={"username": username, "password": password}
            )
            ...
            response = await client.get(
                config.MENU_API_URL, headers={"Authorization": f"Bearer {token}"}
            )
            if response.status_code != 200:
        ...
        if function_name == "add_to_order":
            ...
            feedback, new_order = parse_and_search_order(
                f"{quantity} {item_name}", restaurants, selected_restaurant
            )
            if new_order:
        ...
        if function_name == "confirm_order":
            if not order:
                return (
                    "There's no order to confirm. Please add items to your order first."
                )
            summary = generate_order_summary(order, restaurants)
            response = await client.get(
                config.USERS_API_URL.format(user_id),
            ...
            logger.info(
                f"Order confirmed for session {session_id}, Order ID: {order_data['order_id']}"
            )
            return response
        ...
        if function_name == "cancel_order":
            ...
            order.clear()
            save_state(
                session_id, order, restaurants, False, user_id, token, None, None
            )
