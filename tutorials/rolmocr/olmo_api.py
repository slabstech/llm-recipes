...
app = FastAPI(title="PDF OCR API", description="API for extracting text from PDF pages using olmOCR")
...
async def extract_text_from_pdf(file: UploadFile = File(...), page_number: int = 1):
    ...
    Returns:
        JSONResponse: The extracted text and metadata.
    """
    try:
        # Validate file type
        ...
        # Return the result
        return JSONResponse(content={"extracted_text": text_output})
